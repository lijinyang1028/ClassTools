...
class RandomToolApp(tk.Tk):
    ...
    def parse_json_to_list(self, data):
        """
        增强版JSON解析：全面支持各类列表元素格式
        1. 纯字符串数组 ["张三","李四"]
        2. 数字、布尔等基础类型元素
        3. 嵌套数组 [[1,2],[3,4]] 递归提取所有元素
        4. 对象数组，自动提取 name/title 等常用姓名字段
        5. 字典+指定键名提取列表
        """
        ...
        def extract_items(obj):
            ...
            if isinstance(obj, list):
                for item in obj:
                    res.extend(extract_items(item))
            elif isinstance(obj, dict):
                # 对象优先提取姓名相关字段
                name_keys = ["name", "姓名", "username", "nick", "title"]
                for nk in name_keys:
                    if nk in obj and obj[nk] and isinstance(obj[nk], (str, int, float)):
                        res.append(str(obj[nk]).strip())
                        return res
                # 无名字段则遍历所有值
                for v in obj.values():
                    res.extend(extract_items(v))
            else:
                # 普通值转为字符串，过滤空内容
                val = str(obj).strip()
                if val:
        ...
        # 1. 用户手动指定键名
        if key_name and isinstance(data, dict):
            if key_name not in data:
                messagebox.showerror("键名错误", f"JSON中不存在键：{key_name}")
                return None
            target_data = data[key_name]
            name_list = extract_items(target_data)
            return name_list

        # 2. 自动探测：根为列表 / 字典
        name_list = extract_items(data)
        if not name_list:
            messagebox.showerror("解析失败", "未提取到有效名单数据，请检查JSON内容或手动指定键名")
            return None
        return name_list
